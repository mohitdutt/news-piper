<template>
    <div title="technology">
         <h2 id="technologytitle">Technology</h2>
        <div class="container" v-for="(article, index) in arrcat_tech" v-bind:key="index" @mouseover="displayiconshow(index)" @mouseleave="displayiconhide(index)">
        ...
        <modal-view v-if="this.$store.state.vmmodalstoreT"></modal-view>
        <div class="row" style="margin-right:0">
            <div class="col-md-3" style="margin-left:9%">
                    <mdb-btn color="info" class="previousT" v-bind:class="{ disabled: isDisabledprevious}" v-on:click="previousT()">Previous</mdb-btn>
                </div>
                <div class="col-md-7">
                    <mdb-btn color="info" class="nextT" v-bind:class="{ disabled: isDisablednext}" v-on:click="nextT()">Next</mdb-btn>
                </div>
        </div>
    </div>
</template>
...
import {
    ...
    export default {
        ...
        methods: {
           ...
           technology(){ 
               this.CATEGORY_NEWS({'this': this, cat: 'technology'})
           },
            seen(index){  
           ...
           nextT(){  
              ...
              this.$store.state.arrcat_tech = []
              for(var i =0; i<5; i++){
                if(i+1+this.$store.state.pointetech === this.$store.state.articlescat_tech.length){
                    this.isDisablednext = true
                }else if(i+1+this.$store.state.pointetech < this.$store.state.articlescat_tech.length){
                    this.$store.state.arrcat_tech.push(this.$store.state.articlescat_tech[i+this.$store.state.pointetech])
                }
              }
              this.$store.state.pointetech+= 5
           },
           previousT(){  
             this.isDisablednext = false
             this.$store.state.arrcat_tech = []
             for(var i =0; i<5; i++){ 
               this.$store.state.arrcat_tech.push(this.$store.state.articlescat_tech[i+this.$store.state.pointetech-10])
             }
             this.$store.state.pointetech-= 5
             if(this.$store.state.pointetech === 5){
           ...
           this.$store.state.vmmodalstoreT = true
