<template>
    <div id="sports">
         <h2 id="sportstitle">Sports</h2>
        <div class="container" v-for="(article, index) in arrcat_sports" v-bind:key="index" @mouseover="displayiconshow(index)" @mouseleave="displayiconhide(index)">
        ...
        <modal-view v-if="this.$store.state.vmmodalstoreS"></modal-view>
        <div class="row" style="margin-right:0">
            <div class="col-md-3" style="margin-left:9%">
                    <mdb-btn color="info" class="previousS" v-bind:class="{ disabled: isDisabledprevious}" v-on:click="previousS()">Previous</mdb-btn>
                </div>
                <div class="col-md-7">
                    <mdb-btn color="info" class="nextS" v-bind:class="{ disabled: isDisablednext}" v-on:click="nextS()">Next</mdb-btn>
                </div>
        </div>
    </div>
</template>
...
import {
    ...
    export default {
        ...
        methods: {
            ...
            sports(){
                this.CATEGORY_NEWS({'this': this, cat: 'sports'})
           },
            seen(index){  
           ...
           nextS(){ 
              ...
              this.$store.state.arrcat_sports = []
              for(var i =0; i<5; i++){
                if(i+1+this.$store.state.pointersports === this.$store.state.articlescat_sports.length){
                    this.isDisablednext = true
                }else if(i+1+this.$store.state.pointersports < this.$store.state.articlescat_sports.length){
                    this.$store.state.arrcat_sports.push(this.$store.state.articlescat_sports[i+this.$store.state.pointersports])
                }
              }
              this.$store.state.pointersports+= 5
           },
           previousS(){ 
             this.isDisablednext = false
             this.$store.state.arrcat_sports = []
             for(var i =0; i<5; i++){ 
               this.$store.state.arrcat_sports.push(this.$store.state.articlescat_sports[i+this.$store.state.pointersports-10])
             }
             this.$store.state.pointersports-= 5
             if(this.$store.state.pointersports === 5){
           ...
           this.$store.state.vmmodalstoreS = true
